...
class Model extends CI_Model
{
    ...
    function get_query_string_annual($data,$columns){
        ...
        $query_string = "
            SELECT * FROM (
                SELECT 
                    tb_annual_leave.*,
                    reason.value_name AS reason_text,
                    approve_status.value_name AS approve_text
                FROM tb_annual_leave
                LEFT JOIN m_mini_list AS reason
                    ON reason.value=tb_annual_leave.reason AND reason.`group`='REASON'
    ...
    function getCountLeave($data){
        ...
        $arr['total_leave_default'] = $data_row1->max_annual_leave;
        $arr['total_leave'] = $countLeave;
