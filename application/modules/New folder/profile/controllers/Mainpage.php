...
class Mainpage extends CI_Controller {
    ...
    public function index(){
        ...
        $data['position'] = $this->global_model->general_combo($mini_list,'yes','id','name','');
            ...
            if (!$this->upload->do_upload($file_element_name)){
                ...
                if(file_exists($file_path)){
                    ...
                    if ($jenis_file=='foto') {
                        ...
                        if ($config3['rotation_angle']==90 || $config3['rotation_angle']==270) {
                            $this->image_lib->clear();
                            $config4['image_library'] = 'gd2';
                            $config4['source_image'] = 'uploads/'.$kategori.'_dir/'.$file_upload;
                            $config4['maintain_ratio'] = FALSE;
                            $config4['rotation_angle']  = 'vrt';
                            ...
                            $config5['source_image'] = 'uploads/'.$kategori.'_dir/'.$file_upload;
                            $config5['maintain_ratio'] = FALSE;
                            $config5['rotation_angle']  = 'hor';
                        ...
                        if ($config3['rotation_angle']==90 || $config3['rotation_angle']==270) {
                            $config1['height']  = intval(substr($arr_ko[2], 0, -2));
                            $config1['width'] = intval(substr($arr_ko[3], 0, -2));
                        }
                        else{
                        ...
                        if ($config3['rotation_angle']==90 || $config3['rotation_angle']==270) {
                            $config2['x_axis'] = abs(intval(substr($arr_ko[1], 0, -2)));
                            $config2['y_axis'] = abs(intval(substr($arr_ko[0], 0, -2)));
                        }
                        else{
