...
class Dataclaim extends CI_Controller {
    ...
    function add_data(){
        ...
        $date_claim=date_create($data_row1->date_claim);
        $data_post_claim['month_overtime'] = date_format($date_claim,"m/Y");
        $data_post_claim['sSearch'] = '1';
        $claimCount = $this->model->getCountClaim($data_post_claim);

        if ($_POST['approve_status']=='1') {
            if ($this->global_model->formatHours($claimCount['total_claim']) < $this->global_model->formatHours($claimCount['total_overtime'])) {
    ...
    function show_data(){
        ...
        $arr['filed'] = $claimCount['user_name'];        
        $arr['leave_info'] = "*** INFO : ".$claimCount['user_name']." still has ".$claimCount['overtime_left']." times from a total of ".$claimCount['total_overtime']." times claim for this month";
