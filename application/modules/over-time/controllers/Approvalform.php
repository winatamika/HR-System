...
class Approvalform extends CI_Controller {
    ...
    public function list_data(){
        ...
        $html = '{
            ...
            foreach ($list_data as $row) {
                ...
                $tot_time = ($row->approve_status2=='1'?'<br>Time Approve : <b>'.$this->global_model->view_time($row->valid_time).'</b>':'');
                $leftover = ($row->approve_status2=='1'?' Claim leftover : <b>'.$this->model->get_leftover($row->id).'</b>':'');
    ...
    function add_data(){
        ...
        if (isset($_POST['approve_status1']) || (isset($_POST['approve_status2']) && $_POST['approve_status2']!=1)) {
            $_POST['approve_by1'] = $user->id_user;
            $_POST['approve_date1'] = $date." ".$time;    
            $_POST['valid_period'] = NULL;
            $_POST['valid_start'] = NULL;
            $_POST['valid_end'] = NULL;
        }
        else{
            $_POST['approve_by2'] = $user->id_user;
            $_POST['approve_date2'] = $date." ".$time;
            $_POST['valid_period'] = '2';
            $_POST['valid_start'] = $date." ".$time;
            $_POST['valid_end'] = date('Y-m-d', strtotime("+2 months", strtotime($date)));
        }
        ...
        if ($_POST['id']=='') {
            ...
            if (isset($_POST['approve_status2']) && $_POST['approve_status2']==1) {
                $data_row1 = $this->global_model->general_select('tb_over_time',array('`id`'=>$_POST['id']),'row','','');
                $data_post1['valid_time'] = $this->model->get_time_difference($data_row1->actual_end, $data_row1->actual_start);
                $this->global_model->general_update($data_post1,'tb_over_time',array('id'=>$_POST['id']));
            }
        }
